...
export function EmployeeDocumentsPage() {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create document record using self-service endpoint
      await documentService.uploadMyDocument({
  ...
  const handleDelete = async (doc: EmployeeDocument) => {
    ...
    try {
      // Use self-service endpoint for deleting own documents
      await documentService.deleteMyDocument(doc.id);
      toast.success('Dokumen berhasil dihapus');
      setShowDetailModal(false);
      fetchDocuments();
    } catch (error: any) {
      console.error('Failed to delete document:', error);
      toast.error(error.response?.data?.message || error.response?.data?.error?.message || 'Gagal menghapus dokumen');
    }
  };
          ...
          <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-blue-100 text-sm">View and manage your personal documents</p>
              </div>
            </div>
          ...
          <nav className="flex -mb-px">
              ...
              <div className="flex items-center justify-center gap-2">
                ...
                {stats.fromHR > 0 && (
                  <span className={`px-2 py-0.5 rounded-full text-xs ${
                    activeTab === 'from-hr' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'
                  }`}>
                    {stats.fromHR}
                  </span>
                )}
            ...
            >
              <div className="flex items-center justify-center gap-2">
                ...
                {stats.myUploads > 0 && (
                  <span className={`px-2 py-0.5 rounded-full text-xs ${
                    activeTab === 'my-uploads' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'
                  }`}>
                    {stats.myUploads}
                  </span>
                )}
                ...
                <div>
                  ...
                  <div
                    ...
                    className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors ${
                      uploadForm.file ? 'border-blue-300 bg-blue-50' : 'border-gray-200 hover:border-blue-300'
                    }`}
                  >
                    {uploadForm.file ? (
